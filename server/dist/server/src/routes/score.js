...
router.post('/', async (req, res) => {
    ...
    const hasInsurance = Array.isArray(profile.insurance) &&
    ...
    const investmentSet = new Set((profile.investments ?? []).map((v) => (typeof v === 'string' ? v.trim().toLowerCase() : '')));
    const investments = (() => {
        if (investmentSet.size === 0 || (investmentSet.size === 1 && investmentSet.has('none')))
            return 10;
        if (investmentSet.size === 1 && investmentSet.has('fd'))
            return 40;
        if (investmentSet.size === 1 && investmentSet.has('mutual funds'))
            return 70;
        if (investmentSet.size === 1 && investmentSet.has('stocks'))
            return 80;
        if (investmentSet.size === 1 && investmentSet.has('ppf'))
            return 55;
        if (investmentSet.size === 1 && investmentSet.has('crypto'))
            return 30;
        if (investmentSet.has('stocks') && investmentSet.has('mutual funds') && investmentSet.size === 2)
            return 90;
        if (investmentSet.size >= 3)
            return 100;
        if (investmentSet.has('mutual funds'))
            return 75;
        return 60;
    })();
    const debt = 80;
